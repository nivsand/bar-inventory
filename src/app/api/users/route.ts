  ...
  try {
    ...
    return ok(
      await prisma.user.findMany({
        select: { id: true, name: true, email: true, role: true, area: true, isActive: true, locale: true },
        orderBy: { name: "asc" },
      })
    );
  } catch (e) {
...
export async function POST(req: Request) {
  try {
    ...
    const u = await prisma.user.create({
      data: { name: data.name, email: data.email, role: data.role, area: data.area ?? null, passwordHash },
    });
